"""
Persistence context variables for run_id, request_id, cycle.

Thread-safe via contextvars. Used by sink to correlate llm_calls.
"""
...
def set_current_run_id(run_id: str) -> None:
    """Sets the current run ID for persistence."""
    _run_id.set(run_id)
...
def set_current_request_id(request_id: str) -> None:
    """Sets the current request ID for persistence."""
...
def set_current_cycle(cycle: int) -> None:
    """Sets the current deliberation cycle for persistence."""
    _cycle.set(cycle)
