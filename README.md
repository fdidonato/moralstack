...
### SDK model resolution (important)
...
- The model passed in `client.chat.completions.create(model="...")` controls only the final response generated by your wrapped client.
- `OPENAI_MODEL` and `MORALSTACK_*_MODEL` variables control only the internal governance pipeline.
...
In this example, `gpt-5.2` is used for final response generation only (when allowed by routing). Internal governance still uses
`OPENAI_MODEL` and module-specific overrides.

| Stage | Model source in SDK mode |
|---|---|
| Final response (`NORMAL_COMPLETE`) | `model=` passed to `chat.completions.create(...)` |
| Final response (`SAFE_COMPLETE`) | same `model=` as above, with governance constraints injected into the system message |
...
| Risk / Critic / Simulator / Perspectives / Hindsight | `MORALSTACK_*_MODEL` for each module, fallback to `OPENAI_MODEL` |

Exception: when `final_action` is `REFUSE`, your wrapped client is not called for generation.
