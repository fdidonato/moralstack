...
| SDK/proxy final text source intentionally differs on `COMPLIANCE_FAST_PATH`: proxy may return the governed draft when the guard allows it; SDK still calls the wrapped client with the original full messages | `server/proxy.py:_handle_chat_completion_sync`; `sdk/wrapper.py:_create_inner` | High | Governance decision is shared in `process()`; divergence is post-governance delivery only |
